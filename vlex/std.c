...
struct collect_out collect_immediate(char * _str) {
    ...
    avsme char_class = charclass(_str[0]);
    char * current_token = _str++;
...
size_t print_out_collective(struct collect_out out) {
    size_t count = (size_t)(out.new_token - out.old_token);

    for (int i = 0; i < count; i++) printf("%d", out.char_class);
                                        ...
                                        )
{
    ...
    avsme char_class = charclass(*_str);
    avsme ret_char_class = char_class;
    hash_push *(_str);
    avsme new_char_class;
    char * current_token = _str;
    avsme check_value;
    avsme check_key;
        ...
        for (int i = 0; i < n_keys; i++) {
            ...
            if ( check_key_bool || AVSME_OVERLAP(check_key, char_class) )
            {
                ...
                for (int j = 2; j < table[i*_stride + 1] + 2; j++) {
                    ...
                    else if (j == table[i*_stride + 1] + 1) VLU_NO_VALUE_HANDLE;
            ...
            else if (i == n_keys - 1) VLU_NO_KEY_HANDLE;
